...
def spectest_ticket(db:Session,user:schemas.JWTUser):
    db_ticket = models.Ticket(id=ulid.new().str,group_id="spectest0426",event_id="fakeevent",owner_id=auth.user_object_id(user),person=1,is_used=False,created_at=datetime.now(timezone(timedelta(hours=+9))).isoformat())
    db.add(db_ticket)
    db.commit()
    db.refresh(db_ticket)
    return db_ticket
